...
class BezierBase:
    ...
    def __init__(self, N=2):
        ...
        # 基点坐标
        self.basePoint_pos = ti.Vector.field(2,
                                             dtype=ti.f32,
                                             shape=self.basePosNum)
        # 贝塞尔曲线坐标
        self.bezierCurve_pos = ti.Vector.field(2,
                                               dtype=ti.f32,
                                               shape=self.t_num)
    ...
    def setRandomBasePointPos(self):
        for i in range(0, self.basePosNum):
            self.basePoint_pos[i] = ti.Vector(
                [ti.sqrt(ti.random()) * 0.9,
                 ti.sqrt(ti.random()) * 0.75])
        self.sortBasePoint()
    ...
    def computeBezier(self, u: ti.i32):
        ...
        for k in range(0, self.degree + 1):
            self.bezierCurve_pos[
                u] += self.basePoint_pos[k] * self.computeBinomialCoeff(
                    self.degree, k) * uStep**k * (1 - uStep)**(self.degree - k)

    @ti.func
    def computeBinomialCoeff(self, n: ti.i32, k: ti.i32):
        return self.computeFactorial(n) / (self.computeFactorial(k) *
                                           self.computeFactorial(n - k))
    ...
    def displayBasePoint(self, gui, radius=1, color=0x5e60d3):
        for i in range(0, self.degree):
            gui.line(self.basePoint_pos[i].to_numpy(),
                     self.basePoint_pos[i + 1].to_numpy(),
                     radius=radius,
                     color=color)
        gui.circles(self.basePoint_pos.to_numpy(),
                    radius=radius + 3,
                    color=0xff6401)

    # 绘制曲线
    def displayMidPoint(self, gui, radius=1.5, color=0xffffff):
        gui.circles(self.bezierCurve_pos.to_numpy(),
                    color=color,
                    radius=radius)
